...
- **Environments without IdC (pure IAM User)**: if no IdC instance exists, provision-ps **rejects with a 409** (it does not crash). In such environments, LP2PS still provides the **analysis and generation** of "what the least privilege is," and application is performed by a person via either (a) migrating to PS after adopting IdC, or (b) applying the generated policy directly to IAM. In other words, **PS automation presupposes IdC**, and adopting IdC itself is the goal LP2PS aims for (IAM User → IdC PS migration).
